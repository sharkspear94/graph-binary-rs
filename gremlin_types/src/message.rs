...
use crate::graph_binary::{Decode, Encode, GremlinTypes, MapKeys};
...
pub struct Request {
    ...
    op: String,
    processor: String,
    args: HashMap<MapKeys, GremlinTypes>,
}
...
impl RequestBuilder {
    ...
    pub fn bytecode(mut self) -> BytecodeBuilder {
        ...
        self.0.args.insert(
            ...
            GremlinTypes::Map(HashMap::from([("g".into(), "g".into())])),
        );
...
impl EvalBuilder {
    pub fn bindings(mut self, bindings: HashMap<String, GremlinTypes>) -> Self {
    ...
    pub fn aliases(mut self, aliases: HashMap<String, String>) -> Self {
        if let Some(GremlinTypes::Map(map)) = self.0.args.get_mut(&"aliases".into()) {
    ...
    pub fn alias(mut self, source: &str, alias: &str) -> Self {
        if let Some(GremlinTypes::Map(map)) = self.0.args.get_mut(&"aliases".into()) {
            ...
            self.0.args.insert(
                "aliases".into(),
                HashMap::<MapKeys, GremlinTypes>::from([(source.into(), alias.into())]).into(),
            );
        }
        self
    }
...
pub struct Response {
    ...
    status_attribute: HashMap<MapKeys, GremlinTypes>,
    result_meta: HashMap<MapKeys, GremlinTypes>,
    result_data: GremlinTypes,
}
...
impl Default for Response {
    fn default() -> Self {
        Self {
            ...
            result_data: GremlinTypes::UnspecifiedNullObject,
        }
    }
}
...
impl Response {
    ...
    pub fn result_data(&self) -> &GremlinTypes {
        &self.result_data
    }

    pub fn unwind_traverser(&self) -> Result<Vec<&GremlinTypes>, DecodeError> {
        match &self.result_data {
            GremlinTypes::List(l) => Ok(l
                .iter()
...
impl ResponseBuilder {
    ...
    pub fn result_meta(mut self, result_meta: HashMap<MapKeys, GremlinTypes>) -> ResponseBuilder {
        self.resp.result_meta = result_meta;
        self
    }

    fn result_data(mut self, result_data: GremlinTypes) -> ResponseBuilder {
...
impl Display for Response {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.status_code {
            200 => writeln!(f, "{}", self.result_data()),
            500 => {
                ...
                writeln!(
                    f,
                    "status_message: {}",
                    self.status_message.as_ref().map_or_else(|| "", |s| s)
                )?;
                let v = self
                    .status_attribute
                    .get(&"exceptions".into())
                    .and_then(|e| e.exceptions())
                    .unwrap_or_default();
                writeln!(f, "exceptions: {v}")?;
                let v = self
                    .status_attribute
                    .get(&"stackTrace".into())
                    .and_then(|g| g.get_ref::<str>())
                    .unwrap_or_default();
                writeln!(f, "stackTrace : {}", v)
            }
            _ => todo!(),
        }
    }
...
impl Decode for Response {
        ...
        Self: std::marker::Sized,
    {
        ...
        let status_attributes = HashMap::<MapKeys, GremlinTypes>::partial_decode(reader)?;
        let result_meta = HashMap::<MapKeys, GremlinTypes>::partial_decode(reader)?;
        let result_data = GremlinTypes::decode(reader)?;
...
fn request_message_test() {
    ...
    args.insert(
        ...
        GremlinTypes::String("g.V(x)".to_string()),
    );
    args.insert(
        MapKeys::String("language".to_string()),
        GremlinTypes::String("gremlin-groovy".to_string()),
    );
...
fn print_msg() {
    ...
    args.insert(
        MapKeys::String("gremlin".to_string()),
        GremlinTypes::String("g.V(x).values('age')".to_string()),
    );
    args.insert(
        MapKeys::String("language".to_string()),
        GremlinTypes::String("gremlin-groovy".to_string()),
    ...
    bindings.insert(
        ...
        GremlinTypes::String("1".to_string()),
    );
...
fn test() {
    ...
    let mut args: HashMap<MapKeys, GremlinTypes> = HashMap::<MapKeys, GremlinTypes>::from([
    ...
    args.insert(
        "bindings".into(),
        HashMap::<String, GremlinTypes>::from([("x".into(), 1_i32.into())]).into(),
    );
    args.insert(
        "aliases".into(),
        HashMap::<String, GremlinTypes>::from([
            ("g".into(), "social".into()),
            ("t".into(), "test".into()),
        ])
        .into(),
